...
int main(){
	Point A, B, C, D , E , F;
	while( scanf("%lf%lf %lf%lf %lf%lf %lf%lf %lf%lf %lf%lf", &A.x,&A.y,&B.x,&B.y,&C.x,&C.y,&D.x,&D.y,&E.x,&E.y,&F.x,&F.y ) == 12){
		if ( A.x == 0 && A.y == 0 && B.x == 0 && B.y == 0 && C.x == 0 && C.y == 0 && D.x == 0 && D.y == 0 && E.x == 0 && E.y == 0 && F.x == 0 &&  F.y == 0 ) break;
	
		double k = (fabs(ccw( D, E, F )) / 2)/ fabs(ccw( A,C,B)) ;
		Point ca = C - A;
		Point H =  (ca*k) + A;

		Point p = ( H + B ) ;

		Point G ( p.x - A.x ,p.y - A.y);
