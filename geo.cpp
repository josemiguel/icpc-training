...
pair<double, double > solve( double a, double b, double c ){
	...
	return make_pair( ( -b + sq ) / (2*a), ( -b - sq ) / (2*a) );
}

pair<double, double > intCirc( double x, double y, double dx, double dy, double cx, double cy , double r){

	double a =  ( dx*dx + dy*dy ) ;
	double b = ( 2*x*dx + 2*y*dy - 2*cx*dx - 2*cy*dy );
	double c = ( x*x + y*y + cx*cx + cy*cy - r*r - 2*x*cx - 2*y*cy );
	return  solve( a ,b, c );

}

Point max_slope( Point p1, Point p2, Point piv ){
	int t;
...
int main(){
	...
	while(scanf("%d",&n) && n != -1 ){
		...
		map<Point,set<Point> > mp;
		map<pair<Point,Point>,set<Point> > mpseg;
		int it = 0;
		for( int i = 0; i < n; i++ ){
			int I,D,H;
			scanf("%d%d%d",&I,&D,&H);
			...
			mpseg[ make_pair(izq,punta) ].insert( izq );
			mpseg[ make_pair(izq,punta) ].insert( punta );

			mpseg[ make_pair(der,punta) ].insert( der );
			mpseg[ make_pair(der,punta) ].insert( punta );
		...
		for( int i = 0; i < seg.size(); i++ ){
			for( int j = i + 1; j < seg.size(); j++ ){
				if ( intersect( seg[i].first, seg[i].second, seg[j].first, seg[j].second ) ){
					...
					mp[cross].insert( seg[j].second ); 
					mp[cross].insert( seg[i].second ); 
					mp[seg[i].first].insert( cross );
					mp[seg[j].first].insert( cross );
					it++;

					mpseg[ seg[i] ].insert( cross ) ;
					mpseg[ seg[j] ].insert( cross ) ;

				}
			}
		}

		for( map<pair<Point,Point>,set<Point> > :: iterator t = mpseg.begin() ; t != mpseg.end(); t++){
			set<Point> tmp = t->second;
			for( set<Point> :: iterator i = tmp.begin(); i != tmp.end(); i++){
				for( set<Point> :: iterator j = tmp.begin(); j != tmp.end(); j++){
					if ( j->x > i->x )
						mp[*i].insert( *j );
				}
		}
		...
		Point coge = mp.begin()->first;
		Point prev = coge;
		res.push_back( coge );
//		cout << coge.x << " " << coge.y <<"tiene " << mp[coge].size() << endl;
		while( it-- ){
			vector<Point> pos( mp[prev].begin(), mp[prev].end() );
			if ( pos.size() == 0 ) break;
			bool first = true;
			for( int i = 0; i < pos.size(); i++ ){
				if ( pos[i] == prev ) continue;
				if ( pos[i].x < prev.x ) continue;
				if ( first ) {
					coge = pos[i], first = false;
					continue;
				}
				coge = max_slope( coge, pos[ i ], prev );
			}
			res.push_back( coge );
			prev = coge;
//			cout << coge.x << " " << coge.y <<"tiene " << mp[coge].size() << endl;
